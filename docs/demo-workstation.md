...
В development-режиме рядом с переключателем пользователя доступна кнопка
**«Заполнить демо»**. Она создаёт семь полностью синтетических заявок во всех
основных статусах с разными исполнителями и экспертами, комментариями,
отчётами, заключениями и отметками проверки СБ.

Повторное заполнение сначала безвозвратно удаляет все существующие заявки и
связанные с ними данные и файлы. Пользователи при этом не меняются. Эту команду
следует использовать только на локальной демонстрационной базе после
подтверждения в интерфейсе.

То же действие можно выполнить из контейнера backend:

```powershell
docker compose -f compose.yaml exec backend php yii dev/seed-requests
```

Если обязательные demo-пользователи ещё не созданы, сначала выполните
`docker compose -f compose.yaml exec backend php yii dev/seed`.
