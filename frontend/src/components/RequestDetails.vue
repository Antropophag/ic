...
<template>
  <section class="page request-page screen-panel" :class="{ 'screen-panel--active': hasRequestContent }">
    ...
    <article v-if="hasRequestContent && !detailError" class="card object-band request-entity-head">
      ...
      <div class="object-status-row"><span class="badge" :class="selected.tone">{{ selected.status }}</span><details v-if="selected.canSetColor" class="request-color-control"><summary><span class="request-color-dot" :class="selected.color" aria-hidden="true"></span>Цвет</summary><div class="request-color-menu" role="group" aria-label="Цвет заявки в реестре"><button v-for="color in REQUEST_COLORS" :key="color" type="button" :class="{ active: selected.color === color }" :disabled="actions?.colorLoading" @click="setColorMark(color, $event)"><span>{{ colorLabel(color) }}</span><span class="request-color-dot" :class="color" aria-hidden="true"></span></button></div></details></div>
