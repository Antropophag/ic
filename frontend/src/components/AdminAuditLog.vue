...
<template>
  <section class="admin-log" aria-labelledby="admin-tab-audit">
    <form class="admin-filters" @submit.prevent="submit"><label>Пользователь ID<input v-model="filters.actorId" type="number" min="1"></label><label>Тип события<input v-model.trim="filters.eventType"></label><label>Объект<select v-model="filters.entityType"><option value="">Все</option><option value="request">Заявка</option><option value="user">Пользователь</option></select></label><label>Заявка<input v-model="filters.requestId" type="number" min="1"></label><label>Результат<select v-model="filters.result"><option value="all">Все</option><option value="success">Успешные</option><option value="denied">Отклонённые</option></select></label><label>С<input v-model="filters.dateFrom" type="date"></label><label>По<input v-model="filters.dateTo" type="date"></label><button class="secondary">Применить</button></form>
