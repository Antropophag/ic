...
import { canStartNow } from '../registry'
...
const processSteps = computed(() => {
  ...
  const terminal = ['В проведении испытаний отказано', 'Заявка отозвана'].includes(request.value?.status)
  return labels.map((label, index) => ({ label, state: terminal ? (index === 0 ? 'done' : 'future') : index < statusIndex ? 'done' : index === statusIndex ? 'current' : 'future' }))
})
...
const hasHeroAction = computed(() => Boolean(request.value && (
  ...
  || request.value.canReject || request.value.canWithdraw || request.value.canDeleteReport || request.value.canSuspend || request.value.canResume
)))
...
<template>
  <ol class="process-timeline"><li v-for="step in processSteps" :key="step.label" :class="step.state"><span class="process-node" aria-hidden="true"></span><b>{{ step.label }}</b><small>{{ step.state === 'current' ? 'Текущий этап' : step.state === 'done' ? 'Завершено' : 'Ожидается' }}</small></li></ol>
  <div v-if="hasHeroAction || actions.actionError.value" class="request-process-action">
    <div class="request-process-action-main"><div class="request-action-context"><span>Следующий шаг</span><b>{{ actionPrompt }}</b></div><div class="request-action-bar">
    ...
    </div><button v-if="actionHelp" ref="helpTrigger" type="button" class="request-action-help" :aria-label="actionHelp.label" :title="actionHelp.label" @click="openHelpDrawer"><AppIcon name="help" :size="16" /></button></div>
