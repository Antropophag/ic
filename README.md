...
Письмо в этом случае реально уходит на указанный адрес, а исходный получатель
и текст сохраняются в теле письма пометкой `[Тест, настоящий получатель: ...]`.
После изменения `.env` пересоберите и перезапустите контейнеры
(`docker compose up -d --build`), чтобы `backend`/`scheduler` подхватили новое
значение — `scheduler` рассылает outbox каждые 60 секунд.
