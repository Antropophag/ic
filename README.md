...
подтверждаются обычным LDAP-входом. Пустая переменная пропускает bootstrap с
успешным завершением; ошибка любого элемента откатывает весь список. При каждом
